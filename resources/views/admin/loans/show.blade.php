            ...
            <div class="flex gap-2">
                ...
                @if ($loan->status == 'pending')
                    <form action="{{ route('admin.loans.approve', $loan->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="btn-success" onclick="return confirm('Approve this loan?')">Approve</button>
                    </form>
                @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="card bg-blue-50 border-blue-200">
                <div class="p-4">
                    ...
                    <p class="text-2xl font-bold text-blue-900">Rp {{ number_format($loan->loan_amount, 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <span class="font-medium ml-2">Rp {{ number_format($loan->total_amount_with_interest, 0, ',', '.') }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Approved By:</span>
                        <span class="font-medium ml-2">{{ $loan->approved_by ? \App\Models\User::find($loan->approved_by)->name : 'N/A' }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Disbursement:</span>
                        <span class="font-medium ml-2">{{ $loan->disbursement_date ? $loan->disbursement_date->format('d M Y') : 'N/A' }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                ...
                @if($loan->installments->count() > 0)
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div class="text-center p-3 bg-blue-50 rounded-lg">
                            ...
                            <p class="text-lg font-bold text-blue-900">Rp {{ number_format($loan->installments->sum('total_due_amount'), 0, ',', '.') }}</p>
                        </div>
                        <div class="text-center p-3 bg-green-50 rounded-lg">
                            <p class="text-sm text-green-600">Paid</p>
                            <p class="text-lg font-bold text-green-900">Rp {{ number_format($loan->installments->sum('amount_paid'), 0, ',', '.') }}</p>
                        </div>
                        <div class="text-center p-3 bg-red-50 rounded-lg">
                            <p class="text-sm text-red-600">Outstanding</p>
                            <p class="text-lg font-bold text-red-900">Rp {{ number_format($loan->installments->sum('total_due_amount') - $loan->installments->sum('amount_paid'), 0, ',', '.') }}</p>
                        </div>
                        <div class="text-center p-3 bg-orange-50 rounded-lg">
                            <p class="text-sm text-orange-600">Overdue</p>
                            <p class="text-lg font-bold text-orange-900">{{ $loan->installments->filter(fn($i) => $i->due_date < now())->count() }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        @foreach($loan->installments->sortBy('installment_number') as $installment)
                            <div class="installment-card border rounded-lg p-4 hover:shadow-md transition-shadow
                                @if($installment->status == 'paid') bg-green-50 border-green-200
                                @elseif($installment->status == 'overdue') bg-red-50 border-red-200
                                @elseif($installment->due_date < now() && $installment->status != 'paid') bg-red-50 border-red-200
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <p class="font-medium">
                                                Rp {{ number_format($installment->total_due_amount, 0, ',', '.') }}
                                                @if($installment->fine_amount > 0)
                                                    <span class="text-red-600 text-sm">(+Rp {{ number_format($installment->fine_amount, 0, ',', '.') }} fine)</span>
                                                @endif
                                            </p>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Installments will appear here once the loan is approved.</p>
                    </div>
                @endif
            </div>
        </div>
